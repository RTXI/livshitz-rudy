...
namespace {
	 ...
	 * @precision  number of mantissa bits used, >= 0 and <= 18
	 */
	inline float powFastLookup (const float         val,
										 const float         ilog2,
...
bool test_PowFast( std::ostream* pOut,
	...
	/// adjustable
	{
		...
		for( dword i = T_START;  i < T_END;  ++i ) {
			for( dword j = 0;  j < J_COUNT;  ++j ) {
				...
				if( (0 == j) && pOut && isVerbose ) {
					*pOut << std::showpos << std::fixed << std::setw(10) << number << std::scientific << std::noshowpos;
					*pOut << "  T " << ptf << " " << ptDif << "\n";
				}
			}
		}
	...
	if( pOut ) *pOut << (isOk ? "--- successfully" : "*** failurefully") << " completed " << "\n\n";
	if( pOut ) pOut->flush();
